...
def process_texts_in_batches(
    ...
    for i in tqdm(range(0, len(dataset_subset), batch_size)):
        ...
        hidden_batch_primary = torch.stack(
            [get_last_token_activations_single(
                text, model) for text in batch_primary]
        )
        ...
        sanitized_output_filepath = re.sub(
            r"[^\x00-\x7F]+", "_", output_filepath)
        ...
        try:
            ...
            logging.error(
                f"Failed to save file to {sanitized_output_filepath}: {e}")
