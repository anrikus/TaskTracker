from tasktracker.training.utils.constants import CONSTANTS_ALL_MODELS, OOD_POISONED_FILE
...
def load_evaluation_data(val_files_clean, val_files_poisoned, num_layers):
    ...
    clean_dataset = ActivationsDatasetDynamicPrimaryText(
        val_files_clean, num_layers=num_layers, root_dir=config.get(
            "activations_ood")
    )
    ...
    for primary, clean_with_text in tqdm(clean_dataset):
        clean_diff.append(
            (clean_with_text - primary).flatten().float().numpy())
    poisoned_diff = []
    for primary, poisoned_with_text in tqdm(poisoned_dataset):
        poisoned_diff.append(
            (poisoned_with_text - primary).flatten().float().numpy())
...
if __name__ == "__main__":
    ...
    for n_layer in LAYERS:
        ...
        train_files = load_file_paths(
            os.path.join(ACTIVATION_FILE_LIST_DIR,
                         "train_files_" + MODEL + ".txt")
        )
        val_files_clean = load_file_paths(
            os.path.join(ACTIVATION_FILE_LIST_DIR,
                         "val_clean_files_" + MODEL + ".txt")
        )
        ...
        pickle.dump(model, open(os.path.join(
            layer_output_dir, "model.pickle"), "wb"))
