...
if config["restart"]:
    ...
    if os.path.exists(checkpoint_file):
        start_epoch, best_roc_auc = load_checkpoint(
            checkpoint_file, model, optimizer)
        print(
            f"Restarting from epoch {start_epoch} with best roc auc {best_roc_auc}")
...
def validation_ood(model, val_files_clean, val_files_poisoned, config):
    ...
    with torch.no_grad():  # Disable gradient computation
        distances_clean = compute_distances_validation(
            model, val_files_clean, config)
    ...
    print(
        f"Training step {global_counter_for_save+1}: ROC AUC on OOD data: {roc_auc}")
    model.train()
    return roc_auc
